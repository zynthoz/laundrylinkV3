...
DASHBOARD_LAYOUT_SETTING_KEY = "dashboard_layout"
...
def set_dashboard_layout(value_json):
    timestamp = _now_str()
    ...
    conn.execute(
        ...
        ON CONFLICT(key) DO UPDATE SET
            ...
            updated_at = excluded.updated_at
        """,
        (DASHBOARD_LAYOUT_SETTING_KEY, str(value_json), timestamp, timestamp),
    )
    conn.commit()
    conn.close()
    return value_json
...
def get_time_shifts():
    """Retrieve the list of time-based shifts from app_settings."""
    conn = get_connection()
    row = conn.execute(
        "SELECT value FROM app_settings WHERE key = ? LIMIT 1",
        ("time_shifts",),
...
def set_time_shifts(shifts_list):
    """Save the list of time-based shifts to app_settings."""
    if not isinstance(shifts_list, list):
        raise ValueError("time shifts must be a list")

    timestamp = _now_str()
    serialized = json.dumps(shifts_list, separators=(",", ":"), sort_keys=True)
    conn = get_connection()
    conn.execute(
        """
        INSERT INTO app_settings (key, value, created_at, updated_at)
        ...
        ON CONFLICT(key) DO UPDATE SET
            ...
            updated_at = excluded.updated_at
        """,
        ("time_shifts", serialized, timestamp, timestamp),
    )
    conn.commit()
    conn.close()
    return shifts_list
