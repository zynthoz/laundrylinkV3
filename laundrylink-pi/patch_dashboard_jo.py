...
def replace_func(func_name, new_code):
    global content
    pattern = r"@dashboard_bp\.route\(\"/job-orders\", methods=\[\"POST\"\]\)\s*@require_auth\s*def " + func_name + r"\(.*?(?=\n@dashboard_bp\.route|\Z)"
...
new_jo_func = """@dashboard_bp.route("/job-orders", methods=["POST"])
@require_auth
def dashboard_create_job_order():
    ...
    machine_orders_payload = data.get("machine_orders")
