    ...
    <div class="open-orders-header" style="flex-wrap: wrap; gap: 12px; margin-bottom: var(--space-md);">
      ...
      <div class="filter-bar" style="margin-bottom: 0;">
        <div class="grid-cols-3" style="display: inline-flex; gap: 4px;">
          ...
          <button class="btn btn-secondary ${currentReportType === 'month' ? 'btn-primary' : ''}" id="btn-report-month" onclick="setReportType('month')" style="padding: 0 16px; min-height: 44px;">
            Calendar
          </button>
        </div>
      ...
      <div class="machine-card-shell" style="height: 100%; display: flex; flex-direction: column;">
        <div class="machine-card-inner" style="flex: 1; min-height: auto; display: flex; flex-direction: column; justify-content: flex-start; padding: var(--space-md);">
          ...
          <div class="table-container" style="flex: 1; overflow: auto; min-height: 380px;">
            <table id="transactions-table-main">
              <thead>
                <tr id="transactions-table-head">
                  <th>Time</th>
                  <th>Machine</th>
                  <th>Customer</th>
                  <th>Method</th>
...
async function loadReportsSummary() {
  ...
  if (currentReportType === "day") {
    ...
    endpoint = `/dashboard/summary/calendar?month=${selectedReportDate.substring(0, 7)}`;
...
function updateReportsUI() {
  ...
  if (thRow) {
    if (isAdmin && !thRow.querySelector(".admin-action-col")) {
      thRow.innerHTML += `<th class="admin-action-col">Action</th>`;
    } else if (!isAdmin && thRow.querySelector(".admin-action-col")) {
      thRow.querySelector(".admin-action-col").remove();
    }
  }
  ...
  if (tableBody) {
    // If it's month view, reportsSummaryData is an array of days. Handle differently.
    if (currentReportType === "month") {
      countBadge.textContent = "Calendar";
      if (!Array.isArray(reportsSummaryData)) {
        tableBody.innerHTML = `<tr><td colspan="${isAdmin ? 6 : 5}" style="text-align: center;">No data for month.</td></tr>`;
        return;
      }
      tableBody.innerHTML = reportsSummaryData.map(day => `
        <tr>
          <td>${day.date}</td>
          <td colspan="3">Total Shifts: ${day.shifts}</td>
          <td style="font-weight: 700; color: var(--accent);">₱${(day.net_sales || 0).toFixed(2)}</td>
          ${isAdmin ? '<td></td>' : ''}
        </tr>
      `).join('');
      return;
    ...
    // Get transactions list (Shift or Day view)
    ...
    if (txList.length === 0) {
      tableBody.innerHTML = `<tr><td colspan="${isAdmin ? 6 : 5}" style="text-align: center; color: var(--text-muted); padding: 2.5rem;">No sales recorded yet.</td></tr>`;
    } else {
          ...
          adminCol = `
            <td>
              <div style="display: flex; gap: 4px;">
                <button onclick="toggleTransactionPayment('${tx.id}', ${!isGcash})" title="Toggle Payment Method" style="background:none; border:none; cursor:pointer; color: var(--text-secondary); padding: 4px;">
                ...
                <button onclick="deleteTransaction('${tx.id}')" title="Delete Transaction" style="background:none; border:none; cursor:pointer; color: var(--danger); padding: 4px;">
...
function setReportType(type) {
  ...
  document.getElementById("btn-report-month").className = `btn btn-secondary ${type === 'month' ? 'btn-primary' : ''}`;
  loadReportsSummary();
}
...
function exportToSheets() {
  ...
  let url = `${CONFIG.PI_BASE_URL}/dashboard/export/sheets`;
  if (currentReportType === 'day') {
    url += `?date=${selectedReportDate}`;
  } else if (currentReportType === 'month') {
    url += `?month=${selectedReportDate.substring(0, 7)}`;
  }
  
  // We can just open this in a new tab, or fetch and download if it's a CSV blob.
  // The backend route '/dashboard/export/sheets' probably returns a CSV file download
  window.open(url, "_blank");
  showNotification("Exporting", "Generating CSV export...", "info");
}

async function deleteTransaction(txId) {
  if (!confirm("Delete transaction " + txId + "? This action is irreversible and affects analytics.")) return;
  const pin = sessionStorage.getItem("adm_pin") || "";
  try {
    const res = await apiFetch("/dashboard/transactions/delete", {
      method: "POST",
      headers: { "X-Admin-Pin": pin },
      body: JSON.stringify({ transaction_id: txId })
    });
    if (res.status === "ok") {
      showNotification("Deleted", "Transaction removed successfully.", "success");
      loadReportsSummary();
    }
  } catch(e) {}
}

async function toggleTransactionPayment(txId, setToGcash) {
  const pin = sessionStorage.getItem("adm_pin") || "";
  try {
    const res = await apiFetch(`/dashboard/transactions/${txId}/payment-method`, {
      method: "POST",
      headers: { "X-Admin-Pin": pin },
      body: JSON.stringify({ is_gcash: setToGcash })
    });
    if (res.status === "ok") {
      showNotification("Updated", "Payment method toggled.", "success");
      loadReportsSummary();
    }
  } catch(e) {}
}
