...
// System Settings (Day Change & Analytics)
async function openSystemSettings() {
  const pin = sessionStorage.getItem("adm_pin") || "";
  try {
    const dcRes = await apiFetch("/dashboard/settings/day-change-time", { headers: {"X-Admin-Pin": pin} });
    const anRes = await apiFetch("/dashboard/settings/analytics", { headers: {"X-Admin-Pin": pin} });
    ...
    container.innerHTML = `
      <div style="background: var(--bg); padding: 14px; border-radius: var(--radius-sm); border: 1px solid var(--border-light);">
        ...
        <form onsubmit="saveSystemSettings(event)">
          <div class="form-group">
            <label class="form-label">Day Change Time (HH:MM)</label>
            <input type="time" id="cfg-day-change" value="${dcRes.day_change_time || '00:00'}" required>
          </div>
          <div class="form-group">
            <label style="display: flex; align-items: center; gap: 8px; font-size: var(--text-sm);">
              <input type="checkbox" id="cfg-inc-svc" ${anRes.include_service_revenue_in_net ? 'checked' : ''} style="width: auto; min-height: auto;">
              Include service (add-on) revenue in net sales
            </label>
          </div>
          <div class="grid-cols-2" style="gap: 8px;">
...
async function saveSystemSettings(e) {
  e.preventDefault();
  const pin = sessionStorage.getItem("adm_pin") || "";
  const dayTime = document.getElementById("cfg-day-change").value;
  const incSvc = document.getElementById("cfg-inc-svc").checked;

  try {
    await apiFetch("/dashboard/settings/day-change-time", { method: "POST", headers: {"X-Admin-Pin": pin}, body: JSON.stringify({ day_change_time: dayTime }) });
    await apiFetch("/dashboard/settings/analytics", { method: "POST", headers: {"X-Admin-Pin": pin}, body: JSON.stringify({ include_service_revenue_in_net: incSvc }) });
    showNotification("Success", "System configuration saved.", "success");
    renderAdminFeatures();
  } catch(e) {}
}
...
async function openEmailSettings() {
  const pin = sessionStorage.getItem("adm_pin") || "";
  try {
    const res = await apiFetch("/dashboard/settings/email", { headers: {"X-Admin-Pin": pin} });
    const container = document.getElementById("admin-panel-content");
    container.innerHTML = `
      <div style="background: var(--bg); padding: 14px; border-radius: var(--radius-sm); border: 1px solid var(--border-light);">
        <h4 style="font-weight: 700; margin-bottom: var(--space-sm);">Email Reporting</h4>
        <form onsubmit="saveEmailSettings(event)">
          <div class="form-group">
            <label class="form-label">Recipient Email</label>
            <input type="email" id="cfg-email-to" value="${res.recipient_email || ''}" placeholder="owner@example.com">
          </div>
          <div class="form-group">
            <label style="display: flex; align-items: center; gap: 8px; font-size: var(--text-sm);">
              <input type="checkbox" id="cfg-email-en" ${res.enabled ? 'checked' : ''} style="width: auto; min-height: auto;"> Enable automated day-end reports
            </label>
          </div>
          <div class="grid-cols-2" style="gap: 8px;">
            <button class="btn btn-primary" type="submit">Save</button>
        ...
        <button class="btn btn-secondary" onclick="testEmailSettings()" style="width: 100%; margin-top: 8px;">Send Test Email</button>
      </div>
    `;
  } catch(e) {}
}
...
async function saveEmailSettings(e) {
  e.preventDefault();
  const pin = sessionStorage.getItem("adm_pin") || "";
  const recipient = document.getElementById("cfg-email-to").value;
  const enabled = document.getElementById("cfg-email-en").checked;
  try {
    await apiFetch("/dashboard/settings/email", { method: "POST", headers: {"X-Admin-Pin": pin}, body: JSON.stringify({ recipient_email: recipient, enabled: enabled }) });
    showNotification("Success", "Email configuration saved.", "success");
    renderAdminFeatures();
  } catch(e) {}
}
...
async function testEmailSettings() {
  const pin = sessionStorage.getItem("adm_pin") || "";
  try {
    const res = await apiFetch("/dashboard/settings/email/test", { method: "POST", headers: {"X-Admin-Pin": pin} });
    showNotification("Email Sent", res.message || "Test email dispatched.", "success");
  } catch(e) {}
}

// Promos Manager
async function openPromosManager() {
  try {
    const res = await apiFetch("/catalog/promos");
    ...
    container.innerHTML = `
      <div style="background: var(--bg); padding: 14px; border-radius: var(--radius-sm); border: 1px solid var(--border-light);">
        <h4 style="font-weight: 700; margin-bottom: var(--space-sm);">Promotions</h4>
        <div style="margin-bottom: var(--space-md); max-height: 200px; overflow-y: auto;">
          ${promos.length ? promos.map(p => `
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px; border-bottom: 1px solid var(--border-light);">
              <div>
                ...
                <span style="font-size: 11px; color: var(--text-muted);">${p.is_active ? 'Active' : 'Inactive'}</span>
              </div>
              <button class="btn btn-danger" onclick="deletePromo('${p.id}')" style="min-height: 30px; padding: 4px 8px; font-size: 11px;">Remove</button>
            </div>
          `).join('') : '<p style="color: var(--text-muted); font-size: var(--text-sm);">No promos created.</p>'}
        </div>
        <form onsubmit="createPromo(event)" style="border-top: 1px solid var(--border-light); padding-top: 12px;">
          <h5 style="margin-bottom: 8px;">Create Promo</h5>
          <div class="form-group">
            <input type="text" id="pr-name" required placeholder="Promo Name (e.g. Free Dry)">
          </div>
          <div class="grid-cols-2" style="gap: 8px;">
...
async function createPromo(e) {
  ...
  try {
    await apiFetch("/catalog/promos", { method: "POST", body: JSON.stringify({ name: name, active: true }) });
    showNotification("Success", "Promo created.", "success");
    openPromosManager();
  } catch(e) {}
}
...
async function deletePromo(id) {
  ...
  try {
    await apiFetch("/catalog/promos/" + id, { method: "DELETE" });
    showNotification("Deleted", "Promo removed.", "success");
    openPromosManager();
  } catch(e) {}
}
...
// Employee Management
// ----------------------------------------------------
async function openEmployeeManager() {
  const pin = sessionStorage.getItem("adm_pin") || "";
  try {
    const res = await fetch(`${CONFIG.PI_BASE_URL}/employees`);
          ...
          ${employees.length ? employees.map(e => `
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px; border-bottom: 1px solid var(--border-light);">
              ...
              <div style="display: flex; gap: 4px;">
                ...
                <button class="btn btn-danger" onclick="deleteEmployee('${e.id}')" style="min-height: 30px; padding: 4px 8px; font-size: 11px;">Remove</button>
              </div>
            </div>
          `).join('') : '<p style="color: var(--text-muted); font-size: var(--text-sm);">No employees registered.</p>'}
        </div>
        
        <form onsubmit="createEmployee(event)" style="border-top: 1px solid var(--border-light); padding-top: 12px;">
          <h5 style="margin-bottom: 8px;">Register New Employee</h5>
          <div class="form-group">
            <input type="text" id="emp-reg-id" required placeholder="User ID (e.g. jdelacruz)">
          </div>
          <div class="form-group">
            <input type="text" id="emp-reg-name" required placeholder="Display Name">
          </div>
...
async function createEmployee(e) {
  e.preventDefault();
  const pin = sessionStorage.getItem("adm_pin") || "";
  const empId = document.getElementById("emp-reg-id").value;
  const name = document.getElementById("emp-reg-name").value;
  const empPin = document.getElementById("emp-reg-pin").value;
  const role = document.getElementById("emp-reg-role").value;
  
  try {
    const res = await apiFetch("/employees", {
      method: "POST",
      headers: { "X-Admin-Pin": pin },
      body: JSON.stringify({ id: empId, display_name: name, pin: empPin, role: role })
    });
    if (res.status === "ok") {
      showNotification("Success", "Employee registered.", "success");
      openEmployeeManager();
    }
  } catch(e) {}
}
...
async function resetEmployeePin(id) {
  const newPin = prompt("Enter new PIN for " + id + ":");
  if (!newPin) return;
  const pin = sessionStorage.getItem("adm_pin") || "";
  try {
    const res = await apiFetch(`/employees/${id}/pin`, {
      method: "PUT",
      headers: { "X-Admin-Pin": pin },
      body: JSON.stringify({ pin: newPin })
    });
    if (res.status === "ok") {
      showNotification("Updated", "PIN changed successfully.", "success");
    }
  } catch(e) {}
}
...
async function deleteEmployee(id) {
  ...
  try {
    ...
    if (res.status === "ok") {
      showNotification("Deleted", "Employee removed.", "success");
      openEmployeeManager();
    }
  } catch(e) {}
}
        ...
        <div style="margin-bottom: var(--space-md); max-height: 250px; overflow-y: auto;">
          ${products.length ? products.map(p => `
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px; border-bottom: 1px solid var(--border-light);">
              <div>
                <strong style="display: block;">${p.name}</strong>
                <span style="font-size: 11px; color: var(--text-muted);">₱${p.unit_price} | Stock: ${p.stock_on_hand}</span>
              </div>
              <button class="btn btn-danger" onclick="deleteCatalogItem('${p.id}', 'products')" style="min-height: 30px; padding: 4px 8px; font-size: 11px;">Remove</button>
            </div>
          `).join('') : '<p style="color: var(--text-muted); font-size: var(--text-sm);">No products in catalog.</p>'}
        </div>
        
        <form onsubmit="createCatalogItem(event, 'products')" style="border-top: 1px solid var(--border-light); padding-top: 12px;">
          <h5 style="margin-bottom: 8px;">Add New Product</h5>
          <div class="form-group">
            <input type="text" id="cat-reg-id" required placeholder="Item ID (e.g. ariel-sachet)">
          </div>
          <div class="form-group">
            <input type="text" id="cat-reg-name" required placeholder="Display Name">
          </div>
          <div class="grid-cols-2" style="gap: 8px; margin-bottom: 0;">
            <div class="form-group">
              <input type="number" id="cat-reg-price" required placeholder="Price (₱)" min="0">
            </div>
            <div class="form-group">
...
async function createCatalogItem(e, type) {
  e.preventDefault();
  const pin = sessionStorage.getItem("adm_pin") || "";
  const id = document.getElementById("cat-reg-id").value;
  const name = document.getElementById("cat-reg-name").value;
  const price = document.getElementById("cat-reg-price").value;
  const stock = document.getElementById("cat-reg-stock").value;
  
  try {
    const res = await apiFetch(`/catalog/${type}`, {
      method: "POST",
      headers: { "X-Admin-Pin": pin },
      body: JSON.stringify({ id: id, name: name, unit_price: price, stock_on_hand: stock, low_stock_threshold: 5 })
    });
    if (res.status === "ok") {
      showNotification("Success", "Catalog item created.", "success");
      openCatalogEditor();
    }
  } catch(e) {}
}

async function deleteCatalogItem(id, type) {
  if (!confirm("Remove this item?")) return;
  const pin = sessionStorage.getItem("adm_pin") || "";
  try {
    const res = await apiFetch(`/catalog/${type}/${id}`, {
      method: "DELETE",
      headers: { "X-Admin-Pin": pin }
    });
    if (res.status === "ok") {
      showNotification("Deleted", "Item removed.", "success");
      openCatalogEditor();
    }
  } catch(e) {}
}
          ...
          ${customers.length ? customers.map(c => `
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px; border-bottom: 1px solid var(--border-light);">
              <div>
                ...
                <span style="font-size: 11px; color: var(--text-muted);">${c.phone || 'No phone'}</span>
              </div>
            </div>
          `).join('') : '<p style="color: var(--text-muted); font-size: var(--text-sm);">No registered customers.</p>'}
        </div>
        
        <form onsubmit="createCustomerRecord(event)" style="border-top: 1px solid var(--border-light); padding-top: 12px;">
          <h5 style="margin-bottom: 8px;">Add New Customer</h5>
          <div class="form-group">
            <input type="text" id="cust-reg-name" required placeholder="Full Name">
          </div>
          <div class="form-group">
...
async function createCustomerRecord(e) {
  e.preventDefault();
  const pin = sessionStorage.getItem("adm_pin") || "";
  ...
  try {
    ...
    if (res.status === "ok") {
      showNotification("Success", "Customer registered.", "success");
      openCustomerManager();
    }
  } catch(e) {}
}
